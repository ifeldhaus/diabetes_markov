...
results_threshold20_coverage80 <- readr::read_csv('output/results_24034003L_2020-01-19_2228.csv')
results_threshold30_coverage80 <- readr::read_csv('output/results_24033414L_2020-01-19_2327.csv')
...
## Add individual characteristics to results data frame
results20_80 <- merge(results_threshold20_coverage80, population20, by = "id")
results30_80 <- merge(results_threshold30_coverage80, population30, by = "id")

## Fix cost results for base case (error in '12017640L_2020-01-16_1113.csv' and earlier)
# -- costs are the same across HEF coverage rates
# results20_80 <- results20_80 %>% 
#   mutate(oop_medical = ifelse(strategy == "base" & (state == "healthy" | state == "diabetes" | state == "notx" | state == "dm_death" | state == "other_death"), cost_diagnosis,
#                               ifelse(strategy == "base" & (state == "oad" | state == "ins" | state == "nephro" | state == "retino" | state == "neuro" | state == "ang" | 
#                                                              state == "pvd" | state == "mi" | state == "stroke" | state == "fail"), cost_total, oop_medical)),
...
prevalence <- (n_diabetes / inf.fct) / 1e5
...
# ggplot(data = results.costs.plot %>% filter(cost != "Total"), aes(x = strategy, y = value / n_diabetes, fill = cost)) +
#   geom_bar(stat = "identity", position = "stack") + 
#   theme_minimal() + 
#   # scale_fill_manual(values = wes_palette("FantasticFox1", n = 3)) + 
#   scale_fill_manual(values = cbp1) + 
...
#         axis.text = element_text(size = 12),
#         axis.title = element_text(size = 14)) + 
#   labs(x = "Type of cost", y = "Cost per diabetes case (2019 USD)", fill = "Strategy") + 
#   scale_y_continuous(labels = scales::comma)
# 
# # Per HEF beneficiary
# ggplot(data = results.costs.plot %>% filter(cost != "Total"), aes(x = strategy, y = value / n_hef_beneficiaries, fill = cost)) +
#   geom_bar(stat = "identity", position = "stack") + 
#   theme_minimal() + 
#   # scale_fill_manual(values = wes_palette("FantasticFox1", n = 3)) + 
#   scale_fill_manual(values = cbp1) + 
...
#         axis.text = element_text(size = 12),
#         axis.title = element_text(size = 14)) + 
#   labs(x = "Coverage strategy", y = "Cost per HEF beneficiary (2019 USD)", fill = "Type of costs") + 
#   scale_y_continuous(labels = scales::comma)
...
incr.costs.plot$threshold <- factor(incr.costs.plot$threshold,
                                    ...
                                    labels = c("Poorest 20% eligible for HEF", "Poorest 30% eligible for HEF"))
# # Per capita
# ggplot(data = incr.costs.plot %>% filter(cost != "Total"), aes(x = strategy, y = value / nrow(population20), fill = cost)) +
#   geom_bar(stat = "identity", position = "stack") + 
#   theme_minimal() + 
#   facet_wrap(.~threshold) +
#   # scale_fill_manual(values = wes_palette("FantasticFox1", n = 3)) + 
...
#         axis.text = element_text(size = 12),
#         axis.title = element_text(size = 14)) + 
#   labs(x = "Strategy", y = "Incremental cost per capita (2019 USD)", fill = "Type of cost") + 
#   scale_y_continuous(labels = scales::comma) 
...
results.costs.income.20_80 <- results20_80 %>% 
...
incr.costs.income1.20_80 <- results20_80 %>% 
  group_by(strategy, income_quintile) %>% 
  summarise(
    cost_diagnosis = sum(cost_diagnosis) * inf.fct,
    cost_tx = sum(cost_tx) * inf.fct,
...
incr.costs.income2.20_80 <- results20_80 %>% 
  group_by(strategy, income_quintile) %>% 
  summarise(
    cost_diagnosis = sum(cost_diagnosis) * inf.fct,
    cost_tx = sum(cost_tx) * inf.fct,
...
incr.costs.income2.20_80$threshold <- "20%"
incr.costs.income2.20_80$coverage <- "80%"

incr.costs.income3.20_80 <- results20_80 %>% 
  group_by(strategy, income_quintile) %>% 
  summarise(
    cost_diagnosis = sum(cost_diagnosis) * inf.fct,
    cost_tx = sum(cost_tx) * inf.fct,
...
incr.costs.income3.20_80$threshold <- "20%"
incr.costs.income3.20_80$coverage <- "80%"

incr.costs.income4.20_80 <- results20_80 %>% 
  group_by(strategy, income_quintile) %>% 
  summarise(
    cost_diagnosis = sum(cost_diagnosis) * inf.fct,
    cost_tx = sum(cost_tx) * inf.fct,
...
incr.costs.income4.20_80$threshold <- "20%"
incr.costs.income4.20_80$coverage <- "80%"

incr.costs.income5.20_80 <- results20_80 %>% 
  group_by(strategy, income_quintile) %>% 
  summarise(
    cost_diagnosis = sum(cost_diagnosis) * inf.fct,
    cost_tx = sum(cost_tx) * inf.fct,
...
incr.costs.income5.20_80$threshold <- "20%"
incr.costs.income5.20_80$coverage <- "80%"

incr.costs.income <- rbind(incr.costs.income1.20_80, incr.costs.income2.20_80, incr.costs.income3.20_80, incr.costs.income4.20_80, incr.costs.income5.20_80)
